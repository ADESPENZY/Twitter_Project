    ...
    <div className='mt-24 mb-10 w-[80%] mx-auto'>
        <div className="mb-6 flex flex-col items-end">
            ...
            <div className="flex items-center w-full justify-end">
                ...
                <div className="w-[80%] md:w-1/3 border border-gray-400"></div>
            </div>
            <div className='text-end'>
                {data.subtitle.map((text, index) => (
                  <h2 key={index} className='md:text-xl text-gray-400 font-normal'>{text}</h2>
                ))}
            </div>
        </div>
        <div className='flex flex-col md:flex-row items-center gap-5 w-full'>
            {data.cards.map(card => (
              <div key={card.id} className='bg-white rounded-2xl p-4 flex-1 shadow-green-800 border-2 border-green-800 transition-transform duration-300 hover:-translate-y-2'>
